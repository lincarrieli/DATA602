...
class PandasChain:
    ...
    def __init__(self, name): 
        ...
        self.__current_block = pd.DataFrame() # Create a new Block 
    ...
    def __commit_block(self,block): 
        ...
        block_hash = str(self.__prev_hash) + str(self.__id) + str(dt.datetime.now()) + str(self.__current_block.get_size()) + np.random.randint(0,100) # Create block hash
        self.__prev_hash =  block_hash
        self.__chain.append(self.block)
            self.__seq_id += 1
        new_block = self.__current_block
    ...
    def display_block_headers(self): 
        return str(Block.__seq_id) + str(Block.__status) + str(Block.__block_hash) + str(Block.__prev_hash) + str(len(self.__chain))
    ...
    def get_values(self):
        return (self.__transaction[:, 'Value'].values.tolist())
...
class Block:
    ...
    def __init__(self,seq_id,prev_hash): 
        ...
        self.__transactions = pd.DataFrame(columns = ['Timestamp','Sender','Receiver','Value','TxHash']) # Create a new blank DataFrame with set headers
    ...
    def display_header(self): 
        return Block.__dict__.vars()
    
    # 10 pts - This is the interface for how transactions are added
    def add_transaction(self,s,r,v): 
        ts = dt.datetime.now() # Get current timestamp 
        tx_hash = str(ts) + str(self.__transaction['Sender'] ['Receiver'] ['Value']) # Hash of timestamp, sender, receiver, value
        new_transaction = pd.DataFrame(columns = ['Timestamp','Sender','Receiver','Value','TxHash'])# Create DataFrame with transaction data (a DataFrame with only 1 row)
        # Append to the transactions data
        self.add_transaction.append(new_transaction)
    ...
    def get_simple_merkle_root(self): 
        self.__block_hash = str(self.__prev_hash) + str(self.__id) + str(self.__seq_id) + str(np.random.randint(0, 100))
