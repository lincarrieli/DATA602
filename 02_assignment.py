...
def exercise04(more_temperatures, iot_sensor_points, a, b, c, d, e):
    ...
    # ------ Place code below here \/ \/ \/ ------
    temperatures += more_temperatures.tolist()
...
def exercise07(n):
    ...
    # ------ Place code below here \/ \/ \/ ------
    arry = np.array(n)
    unique = np.unique(arry)

    return len(unique) != len(n)
    # ------ Place code above here /\ /\ /\ ------
...
def exercise09():
    ...
    for x in range(0, 10):
        dog_media = r.get(url=url)
        jObject = json.loads(str(dog_media.text))
        dogs.append(jObject["url"])
