...
class Box:
    ...
    def double(self):
        return Box(self.__l * 2, self.__w * 2)
        
    def __eq__(self, other):
        return (self.__l == other.get_length()) and (self.__w == other.get_width())
    
    def print_dim(self):
        print(f"Length: {self.__l}")
        print(f"Width: {self.__w}")
        
    def get_dim(self):
        return (self.__l, self.__w)
        
    def combine(self, other):
        return Box(self.__l + other.get_length(), self.__w + other.get_width())
       
    def get_hypot(self):
        return math.sqrt(self.__l ** 2 + self.__w ** 2)
...
class MangoDB:
    ...
    def __getitem__(self, item):
        return self.__JSON[item]
        
    def display_all_collections(self):
        for obj in collection:
            print(f"collection: {obj}")
            for item in self.__JSON[obj]:
                print(f"{item} self.__JSON[obj][item]")
                
    def add_collection(self, collection_name):
        self.__JSON[collection_name] = {}
        
    def update_collection(self, collection_name, updates):
        for update in updates:
            self.__JSON[collection_name][update] = updates[update]
            
    def remove_collection(self, collection_name):
        del self.__JSON[collection_name]
        
    def list_collections(self):
        for x in self.__JSON:
            print(x)
    
    def get_collection_size(self, collection_name):
        return len(self.__JSON[collection_name])
        
    def to_json(self, collection_name):
        return json.dumps(self.__JSON[collection_name])
...
def exercise01():
    ...
    #- Combine box3 into box1 (i.e. box1.combine())
    box1 = box3.combine(box1)
            
    #- Double the size of box2
    box2 = box2.double()

    #- Combine box2 into box1
    box1 = box2.combine(box1)

    #- Using a for loop, iterate through and print the tuple received from calling box2's get_dim()
    for attr in box2.get_dim():
        print(attr)
        
    #- Find the size of the diagonal of box2
    diagonal = box2.get_hypot()
...
def exercise02():
    ...
    for x in range(len(test_scores)):
        dict[x] = test_scores[x]
    db.update_collection("testscores", dict)
    db.get_collection_size("testscores")
    db.list_collections()
    print(db['default']['uuid'])
    db.wipe()
    print(db['default']['uuid'])
...
def exercise03():
    ...
    # ------ Place code below here \/ \/ \/ ------
    csv_file = pd.read_csv(https://raw.githubusercontent.com/chainhaus/pythoncourse/master/avacado.csv')
    pd.set_option('display.max_rows', csv_file.shape[0]+1)
    print(csv_file)
